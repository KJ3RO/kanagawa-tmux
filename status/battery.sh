show_battery() {
  ...
  local index=$1
  local icon=$(get_tmux_option "@kanawaga_battery_icon" "#{battery_icon}")
  local color=$(get_tmux_option "@kanawaga_battery_color" "$thm_yellow")
  local text=$(get_tmux_option "@kanawaga_battery_text" "#{battery_percentage}")
